...
struct SharedTerm {
    std::string clozeBody;
    std::string expression;
    std::string conjugationExplanation;
    int score;
};
...
bool compare_6(const SharedTerm* lhs, const SharedTerm* rhs) {
    ...
    if (lhs->conjugationExplanation.empty() != rhs->conjugationExplanation.empty()) {
       return lhs->conjugationExplanation.size() > rhs->conjugationExplanation.size();
    }
